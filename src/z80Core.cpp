...
template<typename T>
concept IsByteOrWord = std::is_same<T, uint8_t>::value || std::is_same<T, uint16_t>::value;
...
template<typename opType, typename readType, typename writeType>
...
template<auto operation, auto read1, auto write>
concept IsSingleParamWriteoutOp = IsInvocableOperationSingleParamWithRet<decltype(operation), decltype(read1), decltype(write)>;
...
template <auto operation, auto readO1_OrWrite, auto readO2_OrRead01>
    requires 
        ( IsDualParamNoWriteoutOp<operation, readO1_OrWrite, readO2_OrRead01> ) ||
        ( IsSingleParamWriteoutOp<operation, readO1_OrWrite, readO2_OrRead01> )
void OperationBase() {
    if constexpr( IsDualParamNoWriteoutOp<operation, readO1_OrWrite, readO2_OrRead01> ) {
        operation( readO1_OrWrite(), readO2_OrRead01() );
        std::cout << "Dual param, no ret\n";
    }
    else if constexpr( IsSingleParamWriteoutOp<operation, readO1_OrWrite, readO2_OrRead01> ) {
        readO1_OrWrite( operation( readO2_OrRead01() ) );
...
static inline uint8_t RegisterAccess() {
...
static inline uint16_t RegisterAccess() {
...
template<RegisterSet& registerSet, bool lowByte>
static inline void RegisterAccess( uint8_t data ) {
    if constexpr ( lowByte ) {
...
template<RegisterSet& registerSet>
static inline void RegisterAccess( uint16_t data ) {
...
template<RegisterSet& registerSet, bool lowByte>
static inline uint16_t RegisterByteAccess() {
    if constexpr ( lowByte ) {
        return registerSet.mainRegister.byteRegisters.r1;
    }
    else {
...
template<RegisterSet& registerSet>
static inline uint16_t RegisterWordAccess() {
    return registerSet.mainRegister.word;
}

template<RegisterSet& registerSet>
static inline uint16_t RegisterIndirectAddress() {
    return registerSet.mainRegister.word;
}
...
static inline ReturnType AddressAccess() {
...
template<auto AddressingMode, IsByteOrWord WriteType>
    requires std::is_same<std::invoke_result_t<decltype(AddressingMode)>, uint16_t>::value
static inline void AddressAccess( WriteType data ) {
...
static inline ReturnType AddressAccess() {
    return AddressAccess<RegisterAccess<registerSet>, ReturnType>();
}

// Write 8/16 bit value to address given by register word
template<RegisterSet &registerSet, IsByteOrWord WriteType>
static inline void AddressAccess( WriteType data ) {
    AddressAccess<RegisterAccess<registerSet>>( data );
}


// Dereferencing from addressing modes
template<auto AddressingMode, IsByteOrWord ReturnType>
    requires std::is_same<std::invoke_result_t<decltype(AddressingMode)>, uint16_t>::value
static inline ReturnType DereferenceFromAddress() {
    return MemoryAccess::Read<ReturnType>( AddressingMode() );
}

template<auto AddressingMode, IsByteOrWord WriteType>
    requires std::is_same<std::invoke_result_t<decltype(AddressingMode)>, uint16_t>::value
static inline void WriteToAddress( WriteType data ) {
    MemoryAccess::Write( AddressingMode(), data );
}

template<RegisterSet &registerSet>
static inline void WriteToRegister( uint16_t data ) {
    registerSet.mainRegister.word = data;
}

template<RegisterSet& registerSet, bool lowByte>
static inline void WriteToRegister( uint8_t data ) {
    if constexpr ( lowByte ) {
        registerSet.mainRegister.byteRegisters.r1 = data;
    }
    else {
...
static inline void operationTick() {
    ...
    switch( opcode ) {
        ...
        case 0x01:{
            ...
            OperationBase<Operations::LD16, AddressingModes::RegisterAccess<BC>, AddressingModes::AddressAccess<AddressingModes::ImmediateExtended<0>, uint16_t>>();
            break;
        }
        case 0x02:{
            //TODO - 02,LD (BC),A,7,2,1
            OperationBase<Operations::LD8, AddressingModes::AddressAccess<AddressingModes::RegisterAccess<BC>, uint8_t>, AddressingModes::RegisterAccess<A, true>>();
            break;
        }
        case 0x03:{
        ...
        case 0x06:{
            ...
            OperationBase<Operations::LD8, AddressingModes::WriteToRegister<BC, true>, AddressingModes::Immediate<0>>();
            break;
        }
        case 0x07:{
            //TODO - 07,RLCA,4,1,1
        ...
        case 0x0A:{
            ...
            OperationBase<Operations::LD8, AddressingModes::WriteToRegister<A, true>, AddressingModes::DereferenceFromAddress<AddressingModes::RegisterIndirectAddress<BC>, uint8_t>>();
            break;
        }
        case 0x0B:{
        ...
        case 0x0C:{
            //TODO - 0C,INC C,4,1,1
            OperationBase<Operations::LD8, AddressingModes::WriteToRegister<BC, false>, AddressingModes::Immediate<0>>();
            break;
        }
        case 0x0D:{
            //TODO - 0D,DEC C,4,1,1
