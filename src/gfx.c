...
void gfx_open( int width, int height, const char *title )
{
	...
	XSelectInput(gfx_display, gfx_window, StructureNotifyMask|KeyPressMask|KeyReleaseMask|ButtonPressMask);
...
bool gfx_KeyEvent( uint16_t *keyCode, bool *pressed )
{
	XEvent event;
	gfx_flush();
	
	while( XCheckMaskEvent( gfx_display, KeyPressMask | KeyReleaseMask, &event ) ) {
		if ( event.type == KeyPress ) {
